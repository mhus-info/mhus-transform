...
public class FillModel extends MLog {
    ...
    public FillModel(File file, TransformConfig context)
        ...
        if (shell != null) log().d("variable", shell.getContext().getProperty("vars"));
        else log().d("variable", context.getParameters());
    }
    ...
    public String getValue(String name, PDField field) throws Exception {
        ...
        if (!one && !MString.isEmpty(value)) return value;
